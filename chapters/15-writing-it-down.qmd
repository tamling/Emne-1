...
{"type":"buckets","title":"Try it: one line, or a file?","instr":"Sort each fact onto the form it describes. The first rows of the chapter's table are honest: the shell is good at some of this.",
 "buckets":["A one-line pipeline","A script in a file"],
 "items":[["No memory between the steps",0],["React when a count passes ten: one if",1],["The same job for six files: a loop",1],["Names and lines to read next year",1],["One long line in terminal history",0],["Counting the refusals: comfortable",0]],
 "ok":"Sorted. The turn comes when a question needs a decision or a second pass."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"buckets","title":"Try it: text or number?","instr":"The same character on the screen, two different things inside the machine. Sort each fact onto the value it is true of.",
 "buckets":["The text '5'","The number 5"],
 "items":[["Written inside quotation marks",0],["Added to itself gives 55",0],["Added to itself gives 10",1],["len() answers 1",0],["len() is an error",1],["type() says class int",1]],
 "ok":"Sorted. Adding text joins it, adding numbers adds them, and the plus sign means both."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"match","title":"Try it: take the text apart","instr":"Drag each operation onto what it does. Two chips are distractors and stay on the bench.",
 "zones":[{"label":"Cuts a string at every run of spaces","answer":".split()"},{"label":"Removes whitespace from both ends","answer":".strip()"},{"label":"How many characters","answer":"len()"},{"label":"Does it begin with what you gave it?","answer":".startswith()"}],
 "extra":[".lower()","repr()"],
 "ok":"Those are the operations that take a log line apart. lower() converts case, and repr() only shows what is really in the string."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"quiz","title":"Check yourself","q":"A person types 12 at an input() prompt. What does the script receive?","options":["The string '12'","The integer 12","The float 12.0","True","A ValueError, immediately"],"correct":[0],
 "ok":"Right.","why":"input() always hands back a string, holding the characters that spell the number. Convert it with int() before computing - the book's tip is Don't Wait to Convert."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"order","title":"Try it: four runs, one script","instr":"tellfeil.py was run after each new line was added. Tap the four printed answers in the order they appeared.",
 "steps":["0 - the tally exists","69 - the file is being read","3 - the question works","3 refusals in auth.log - the sentence"],
 "ok":"Write one line, run once, and a failure has one suspect. The known answer was L12's grep -c."}
</script>
```
