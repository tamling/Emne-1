...
{"type": "buckets", "title": "Try it: switch or router?", "instr": "Sort each line of the side-by-side table into the device it describes.", "buckets": ["A switch", "A router"], "items": [["Works inside one network", 0], ["Joins different networks together", 1], ["Reads the address on the card", 0], ["Uses a table of known networks", 1], ["Learns by watching traffic", 0], ["Reduces the hop counter by one", 1]], "ok": "Two different jobs, even when they share one box on the wall.", "why": "A switch forwards by card address inside one network; a router joins networks, using a table and reducing the hop counter.", "ref": "Part 1; Jøsang, Sect. 6.1.2, p. 125"}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type": "quiz", "title": "Check yourself", "q": "A packet for 10.2.5.7 matches 10.0.0.0/8, 10.2.5.0/24 and 0.0.0.0/0 in the same table. Which line does the router use?", "options": ["10.2.5.0/24 - the narrowest matching line wins", "10.0.0.0/8 - the first match in the table", "0.0.0.0/0 - the default route always applies", "All three - the packet goes out of three ports"], "correct": [0], "ok": "Right.", "why": "A longer prefix is a more specific claim and the most specific claim is believed; the default route matches everything, so it is used last.", "ref": "Part 2 of this chapter"}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type": "buckets", "title": "Try it: what changes on the journey?", "instr": "A packet crosses several routers. Sort each field of it.", "buckets": ["Same from end to end", "Changes at every hop"], "items": [["The source IP address", 0], ["The destination IP address", 0], ["The source card (MAC) address", 1], ["The destination card (MAC) address", 1], ["The TTL counter", 1]], "ok": "The IP layer runs end to end; the link layer is rebuilt at every router.", "why": "The IP addresses run end to end; the card addresses are rebuilt for each cable, and every router reduces the TTL by one.", "ref": "Part 3; Jøsang, Sect. 6.1.2, Fig. 6.3, p. 122"}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type": "order", "title": "Try it: NAT, one packet out and back", "instr": "Tap the steps in the order NAT performs them.", "steps": ["Packet leaves with inside address and port", "Router notes the original pair", "Router swaps in public address and port", "Answer returns to the public pair", "Router looks the pair up and swaps back"], "ok": "Arithmetic and a table - no guard anywhere in it.", "why": "The router records the inside pair, substitutes its public address and a chosen port, and reverses the swap when the answer returns.", "ref": "Part 4; RFC 3022"}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type": "quiz", "title": "Check yourself", "q": "A packet from the internet arrives at a NAT router unasked. What stops it?", "options": ["It matches no entry in the NAT table, so it is dropped", "A rule somebody wrote refuses it", "The router inspects it and finds it hostile", "Nothing - unasked packets always pass through"], "correct": [0], "ok": "Right.", "why": "That protection is bookkeeping, not a decision: the router simply does not know where to send it, and one forwarded port removes it.", "ref": "Part 4 of this chapter"}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type": "quiz", "title": "Check yourself", "q": "A traceroute shows asterisks at hop four and an answer at hop five. What does hop four's silence usually mean?", "options": ["That router was told not to reply - a policy, not a fault", "Hop four is broken", "The packet was dropped at hop four", "The packet left Norway at hop four"], "correct": [0], "ok": "Right.", "why": "Hops after it still answer, so the packet passed through; a row of asterisks usually means that router was told not to reply.", "ref": "Parts 3 and 5 of this chapter"}
</script>
```
