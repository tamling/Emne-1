...
```{=html}
<!-- Interactive exercise: drag the three CIA-triad goals onto the triangle.
     Self-contained (style + markup + script); scoped under .cia-widget.
     Dark mode follows Quarto's body.quarto-dark class. -->
<style>
.cia-widget { border: 1px solid #bfc6d4; border-radius: 6px; padding: 1rem 1rem 1.2rem; margin: 1.2rem 0; background: #f8fafb; }
.cia-widget h4 { margin: 0 0 .3rem; font-size: 1rem; color: #0d1f4f; }
.cia-widget .cia-instr { font-size: .9rem; color: #444; margin-bottom: .8rem; }
.cia-widget .cia-stage { position: relative; max-width: 560px; height: 330px; margin: 0 auto; }
.cia-widget svg.cia-tri { position: absolute; inset: 0; width: 100%; height: 100%; }
.cia-widget .cia-zone { position: absolute; width: 168px; min-height: 58px; border: 2px dashed #8d99ae; border-radius: 8px; background: #fff; padding: 4px 6px; text-align: center; transition: border-color .15s, background .15s; }
.cia-widget .cia-zone .cia-cap { font-size: .72rem; color: #5b6b73; font-weight: 600; text-transform: uppercase; letter-spacing: .02em; }
.cia-widget .cia-zone.cia-over { border-color: #d8973c; background: #fbf1e1; }
.cia-widget .cia-zone.cia-filled { border-style: solid; border-color: #0d1f4f; }
.cia-widget .cia-zone.cia-wrong { border-color: #bf3d37; background: #f8eceb; }
.cia-widget .cia-zone.cia-right { border-color: #2e7d32; background: #e8f3e9; }
.cia-widget .cia-tray { display: flex; flex-wrap: wrap; gap: .5rem; justify-content: center; margin-top: .6rem; min-height: 2.2rem; }
.cia-widget .cia-chip { display: inline-block; padding: .3rem .8rem; border: 1px solid #0d1f4f; border-radius: 999px; background: #e9ecf4; color: #0d1f4f; font-size: .88rem; font-weight: 600; cursor: grab; user-select: none; touch-action: none; }
.cia-widget .cia-chip.cia-sel { outline: 3px solid #d8973c; }
.cia-widget .cia-chip.cia-drag { position: fixed; z-index: 999; pointer-events: none; opacity: .9; cursor: grabbing; }
.cia-widget .cia-zone .cia-chip { cursor: pointer; margin-top: 2px; }
.cia-widget .cia-msg { text-align: center; font-size: .92rem; min-height: 1.4rem; margin-top: .7rem; font-weight: 600; }
.cia-widget .cia-msg.ok { color: #2e7d32; }
.cia-widget .cia-msg.no { color: #bf3d37; }
.cia-widget .cia-btns { text-align: center; margin-top: .5rem; }
.cia-widget .cia-reset { display: inline-block; margin: 0 .25rem; font-size: .8rem; padding: .25rem .9rem; border: 1px solid #8d99ae; border-radius: 4px; background: transparent; color: inherit; cursor: pointer; }
body.quarto-dark .cia-widget { background: #23262b; border-color: #4a5568; }
body.quarto-dark .cia-widget h4 { color: #cfe0ff; }
body.quarto-dark .cia-widget .cia-instr { color: #b5bcc4; }
body.quarto-dark .cia-widget .cia-zone { background: #2c3036; border-color: #6b7686; }
body.quarto-dark .cia-widget .cia-zone.cia-over { background: #3a3323; }
body.quarto-dark .cia-widget .cia-zone.cia-filled { border-color: #cfe0ff; }
body.quarto-dark .cia-widget .cia-zone.cia-wrong { background: #3a2626; }
body.quarto-dark .cia-widget .cia-zone.cia-right { background: #223527; border-color: #66bb6a; }
body.quarto-dark .cia-widget .cia-chip { background: #39415a; color: #dfe6f5; border-color: #8fa2d4; }
body.quarto-dark .cia-widget .cia-zone .cia-cap { color: #9fb0b8; }
@media (max-width: 480px) { .cia-widget .cia-zone { width: 128px; } .cia-widget .cia-stage { height: 300px; } }
</style>
<div class="cia-widget" id="cia-triad-ex">
  <h4>Try it: build the CIA triad</h4>
  <p class="cia-instr">Three of the six terms below form the CIA triad. Drag each one onto the corner of the
  triangle that matches the failure it names (or tap a term, then tap a corner). Tap a placed term to take it
  back. The three wrong terms stay on the bench.</p>
  <div class="cia-stage">
    <svg class="cia-tri" viewBox="0 0 560 330" aria-hidden="true">
      <polygon points="280,52 78,268 482,268" fill="none" stroke="#8d99ae" stroke-width="2.5" stroke-linejoin="round"/>
    </svg>
    <div class="cia-zone" data-answer="Availability"    style="left:50%; top:12px; transform:translateX(-50%);"><div class="cia-cap">It stops working</div></div>
    <div class="cia-zone" data-answer="Confidentiality" style="left:2px;  bottom:6px;"><div class="cia-cap">Someone reads it</div></div>
    <div class="cia-zone" data-answer="Integrity"       style="right:2px; bottom:6px;"><div class="cia-cap">Someone changes it</div></div>
  </div>
  <div class="cia-tray" id="cia-tray"></div>
  <div class="cia-msg" id="cia-msg" role="status"></div>
  <div class="cia-btns">
    <button class="cia-reset" id="cia-sol" type="button">Show solution</button>
    <button class="cia-reset" id="cia-reset" type="button">Reset</button>
  </div>
</div>
<script>
(function () {
  var root = document.getElementById('cia-triad-ex');
  var tray = document.getElementById('cia-tray');
  var msg = document.getElementById('cia-msg');
  var zones = Array.prototype.slice.call(root.querySelectorAll('.cia-zone'));
  var TERMS = ['Availability', 'Confidentiality', 'Integrity', 'Authenticity', 'Non-repudiation', 'Privacy'];
  ...
  function setMsg(text, cls) { msg.textContent = text; msg.className = 'cia-msg' + (cls ? ' ' + cls : ''); }

  function reset() {
    solved = false; selected = null;
    zones.forEach(function (z) {
      z.className = 'cia-zone';
      var chip = z.querySelector('.cia-chip');
      if (chip) chip.remove();
    });
    tray.innerHTML = '';
    // shuffle the bench so the order gives nothing away
    TERMS.slice().sort(function () { return Math.random() - 0.5; })
      .forEach(function (t) { tray.appendChild(makeChip(t)); });
    setMsg('', '');
  }

  function place(chip, zone) {
    if (solved || zone.querySelector('.cia-chip')) return;
    chip.classList.remove('cia-sel');
    zone.appendChild(chip);
    zone.classList.add('cia-filled');
    selected = null;
    check();
  }

  function takeBack(chip) {
    if (solved) return;
    var zone = chip.closest('.cia-zone');
    if (zone) zone.className = 'cia-zone';
    tray.appendChild(chip);
    setMsg('', '');
  }

  function check() {
    var filled = zones.filter(function (z) { return z.querySelector('.cia-chip'); });
    if (filled.length < zones.length) return;
    var wrong = [];
    zones.forEach(function (z) {
      var chip = z.querySelector('.cia-chip');
      if (chip.dataset.goal === z.dataset.answer) { z.classList.add('cia-right'); }
      else { z.classList.add('cia-wrong'); wrong.push(chip); }
    });
    if (wrong.length === 0) {
      solved = true;
      setMsg('✓ Correct - confidentiality, integrity and availability form the CIA triad. Authenticity supports all three but is not one of its corners. (See Part 1 and Jøsang, Sect. 1.3, p. 4.)', 'ok');
      return;
    }
    var usedAuth = wrong.some(function (c) { return c.dataset.goal === 'Authenticity'; });
    setMsg(usedAuth
      ? 'Not quite - authenticity is the fourth goal, but it is not a corner of the triad. The red ones went back.'
      : 'Not quite - the red ones went back to the bench. Try again.', 'no');
    setTimeout(function () {
      wrong.forEach(takeBack);
      zones.forEach(function (z) { z.classList.remove('cia-wrong'); });
    }, 900);
  }

  // click-to-select, click-zone-to-place (works everywhere, incl. keyboard)
  root.addEventListener('click', function (e) {
    var chip = e.target.closest('.cia-chip');
    if (chip && chip.closest('.cia-zone')) { takeBack(chip); return; }
    if (chip && !chip.classList.contains('cia-drag')) {
      if (selected) selected.classList.remove('cia-sel');
      selected = (selected === chip) ? null : chip;
      if (selected) selected.classList.add('cia-sel');
      return;
    }
    var zone = e.target.closest('.cia-zone');
    if (zone && selected) place(selected, zone);
  });
  root.addEventListener('keydown', function (e) {
    if (e.key === 'Enter' && e.target.classList && e.target.classList.contains('cia-chip')) e.target.click();
  });

  // pointer-based dragging (mouse and touch)
  var drag = null;
  root.addEventListener('pointerdown', function (e) {
    var chip = e.target.closest('.cia-chip');
    if (!chip || solved || chip.closest('.cia-zone')) return;
    drag = { chip: chip, ghost: null, x0: e.clientX, y0: e.clientY, moved: false };
  });
  document.addEventListener('pointermove', function (e) {
    ...
    if (!drag.ghost) {
      ...
      g.classList.add('cia-drag');
      document.body.appendChild(g);
      drag.ghost = g;
      drag.chip.style.opacity = '0.35';
    }
  ...
  document.addEventListener('pointerup', function (e) {
    ...
    if (!d.moved) return; // plain click: the click handler does selection
    var target = zones.find(function (z) {
      var r = z.getBoundingClientRect();
      return e.clientX >= r.left && e.clientX <= r.right && e.clientY >= r.top && e.clientY <= r.bottom;
    });
    zones.forEach(function (z) { z.classList.remove('cia-over'); });
    if (target) place(d.chip, target);
  });

  var solBtn = document.getElementById('cia-sol'), solArmed = false;
  document.getElementById('cia-reset').addEventListener('click', function () {
    solArmed = false; solBtn.textContent = 'Show solution'; reset();
  });
  solBtn.addEventListener('click', function () {
    if (!solArmed) {
      solArmed = true;
      solBtn.textContent = 'Really show it';
      setMsg('Clicking straight to the answer is no fun - try it first. (Click again if you really want it.)', 'no');
      return;
    }
    solArmed = false; solBtn.textContent = 'Show solution';
    reset();
    solved = true;
    zones.forEach(function (z) {
      var chip = Array.prototype.slice.call(tray.querySelectorAll('.cia-chip'))
        .find(function (c) { return c.dataset.goal === z.dataset.answer; });
      if (chip) { z.appendChild(chip); z.classList.add('cia-filled', 'cia-right'); }
    });
    setMsg('Solution shown - the triad is confidentiality, integrity and availability; authenticity is the supporting fourth goal. See Part 1 and Jøsang, Sect. 1.3, p. 4. Hit Reset to try it properly.', 'ok');
  });
  reset();
})();
</script>
...
{"type": "quiz", "title": "Check yourself", "q": "An organisation buys a firewall. According to the book's four dimensions, what decides whether the purchase is worth anything?", "options": ["People, Partner and Process around the Product", "The price paid for the firewall", "How new the firewall model is", "Whether the staff would ever click a link"], "correct": [0], "ok": "Right.", "why": "A Product without staff who can operate it, a vendor who supports it and a routine for using it has little value.", "ref": "Part 1; Jøsang, Sect. 1.6, pp. 10-11"}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type": "match", "title": "Try it: name the kind of control", "instr": "Drag each kind of control onto its description. Two terms are not kinds of control and stay on the bench.", "zones": [{"label": "Stops incidents; likelihood-reducing", "answer": "Preventive"}, {"label": "Raises alarms; shortens time to action", "answer": "Detective"}, {"label": "Restores operation; impact-reducing", "answer": "Corrective"}, {"label": "Organises the other three; often an ISMS", "answer": "Governance"}], "extra": ["Accountability", "Triage"], "ok": "Right. Authentication and access control prevent, intrusion detection detects, backups and contingency plans correct, and governance organises the three.", "why": "Preventive controls reduce likelihood, detective ones raise alarms, corrective ones restore operation, and governance organises the other three.", "ref": "Part 2; Jøsang, Sect. 1.5, pp. 8-9"}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type": "order", "title": "Try it: the incident management process", "instr": "Tap the book's four phases in the order they run (tap a step, then a slot - or just tap steps in order).", "steps": ["Preparation", "Triage", "Response", "Post-incident"], "ok": "That is the formal loop. The lessons-learned meeting sits in the last phase, and it is what improves the next round of prevention.", "why": "The process runs preparation, triage, response and post-incident; the lessons-learned meeting sits in the last phase.", "ref": "Part 2; Jøsang, Fig. 14.3, p. 308"}
</script>
```
...
::: {.callout-warning title="Before L02"}
...
```{=html}
<script type="application/json" class="qex">
{"type": "buckets", "title": "Try it: who wants what", "instr": "Sort each fact onto the threat actor it describes.", "buckets": ["Criminals", "States", "Hacktivists", "Insiders"], "items": [["Want money above all", 0], ["Automated and loud once inside", 0], ["Want information and lasting access", 1], ["Quiet, patient, well resourced", 1], ["Want attention for a cause", 2], ["Public and short-lived", 2], ["Already hold a key", 3], ["Often an account nobody disabled", 3]], "ok": "Sorted. What an attacker wants predicts their behaviour better than how skilled they are.", "why": "Criminals want money, states want information and lasting access, hacktivists want attention for a cause, and insiders already hold a key.", "ref": "Part 4; Jøsang, Fig. 10.5, p. 237"}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type": "quiz", "title": "Check yourself", "q": "Which of the three February assessments is published by Politiets sikkerhetstjeneste (PST)?", "options": ["Nasjonal trusselvurdering", "Fokus", "Risiko", "All three of them"], "correct": [0], "ok": "Right.", "why": "PST looks inwards with Nasjonal trusselvurdering. Fokus is Etterretningstjenesten looking outwards, and Risiko is NSM on how well protection works in practice.", "ref": "Part 5 of this chapter"}
</script>
```
