...
```{=html}
<script type="application/json" class="qex">
{"type":"quiz","title":"Check yourself","q":"Why does the chapter say you must never write a bare except: with no name?","options":["It catches everything, even your own typos, and the script runs on with wrong data","It is slower than naming the exception","Python refuses to run a bare except","It only works inside a function"],"correct":[0],"ok":"Right.","why":"It even swallows Ctrl-C. Name the exception you can actually do something about, and let the rest reach the traceback."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"buckets","title":"Try it: which stream?","instr":"Sort each line onto the stream the chapter sends it to.","buckets":["stdout","stderr"],"items":[["Results another program can read",0],["Problems, visible even when redirected",1],["Leste 69 linjer",0],["Fant ikke filen: auth.lg",1],["The usage line for a wrong call",1]],"ok":"Results on stdout, problems on stderr - and the exit code is for the scheduler."}
</script>
```

```{=html}
<script type="application/json" class="qex">
{"type":"quiz","title":"Check yourself","q":"You run python3 lesfil_trygg.py with no filename at all. What does echo $? show afterwards?","options":["2","0","1","69"],"correct":[0],"ok":"Right.","why":"Wrong usage exits with code 2, a missing or unreadable file with 1, and a successful run with 0 - three runs, three codes."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"order","title":"Try it: 19 July 2024","instr":"Tap the events of the CrowdStrike case in the order the chapter gives them.","steps":["Template type defines 21 fields, code supplies 20","New content requires the 21st field","The sensor reads memory that is not its own","8.5 million machines stop with a blue screen"],"ok":"The mismatch sat harmless for months - the check that would have caught it is one line: count what arrived."}
</script>
```
