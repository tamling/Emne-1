...
{"type":"quiz","title":"Check yourself","q":"What does a primary key guarantee about a table?","options":["Its value differs in every single row","The rows stay sorted by that column","The column holds only whole numbers","The table can never be emptied"],"correct":[0],"ok":"Right.","why":"A second row with a key that already exists is refused, so another table can point at exactly one row and no other."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"quiz","title":"Check yourself","q":"Why had GitLab's pg_dump backups been failing for months without anyone knowing?","options":["An older tool version produced empty files, and the warning emails went to a folder nobody read","The disk holding them was full","An engineer had deleted the backup job","The database refused every connection"],"correct":[0],"ok":"Right.","why":"Four of five recovery paths had failed, and nobody knew, because nobody had restored from any of them. A backup counts only once it has been restored."}
</script>
```
