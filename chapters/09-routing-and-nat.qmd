...
```{=html}
<script type="application/json" class="qex">
{"type":"buckets","title":"Try it: what changes on the journey?","instr":"A packet crosses several routers. Sort each field of it.","buckets":["Same from end to end","Changes at every hop"],"items":[["The source IP address",0],["The destination IP address",0],["The source card (MAC) address",1],["The destination card (MAC) address",1],["The TTL counter",1]],"ok":"The IP layer runs end to end; the link layer is rebuilt at every router."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"quiz","title":"Check yourself","q":"A packet from the internet arrives at a NAT router unasked. What stops it?","options":["It matches no entry in the NAT table, so it is dropped","A rule somebody wrote refuses it","The router inspects it and finds it hostile","Nothing - unasked packets always pass through"],"correct":[0],"ok":"Right.","why":"That protection is bookkeeping, not a decision: the router simply does not know where to send it. It ends the moment somebody forwards a port."}
</script>
```
