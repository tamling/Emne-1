...
{"type":"buckets","title":"Try it: three ways to look","instr":"Sort each fact onto the tool it belongs to. None of the three opens the whole file.",
 "buckets":["head","tail","less"],
 "items":[["The first ten lines",0],["The last ten lines",1],["One screen at a time",2],["During an incident, start here",1],["A format you have never seen",0],["Space forward, slash searches, q leaves",2]],
 "ok":"Sorted. A log is written downwards, so the newest line is at the bottom."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"match","title":"Try it: five characters","instr":"Drag each pattern character onto what it matches. One character is a distractor: it joins commands, it matches nothing.",
 "zones":[{"label":"The start of a line","answer":"^"},{"label":"The end of a line","answer":"$"},{"label":"Any one character","answer":"."},{"label":"Any number of the one before it","answer":"*"},{"label":"Any single digit","answer":"[0-9]"}],
 "extra":["|"],
 "ok":"Those five cover every example in this chapter. The vertical bar is the pipe from Part 4, not a pattern."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"buckets","title":"Try it: grep or find?","instr":"The two tools never overlap. Sort each fact onto the one it describes.",
 "buckets":["grep","find"],
 "items":[["Reads what is inside files",0],["Never opens a file, reads facts about it",1],["A signature search over a log",0],["Start, a test, an optional action",1],["-c counts the matching lines",0],["-mtime -1: changed in the last day",1]],
 "ok":"Sorted. grep finds lines inside files; find finds the files themselves."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"quiz","title":"Check yourself","q":"Read aloud: what does 2>&1 at the end of a command mean?","options":["Send stream two to wherever stream one is going","Append to the file instead of replacing it","Throw the error messages away","Run the two commands at once","Catch the results and discard the errors"],"correct":[0],
 "ok":"Right.","why":"Results travel on stream one and errors on stream two; a plain arrow only catches stream one, and 2>&1 sends the errors where the results go."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"quiz","title":"Check yourself","q":"Somebody tried to log in as admin, an account this machine has never had. What is the book's name for that sign?","options":["A precursor","An indicator","A signature","A false alarm","An audit"],"correct":[0],
 "ok":"Right.","why":"A precursor is a sign an incident may occur soon; an indicator is a sign one may already have occurred, like the jndi: string during Log4Shell."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"order","title":"Try it: where did they come from","instr":"Tap the pipeline's stages in the order they run to count attempts per address, biggest first.",
 "steps":["grep -oE keeps only the matched addresses","sort brings identical lines together","uniq -c counts each group","sort -rn puts the biggest count first"],
 "ok":"That is the pipeline. uniq only collapses neighbours, which is why sort comes before it."}
</script>
```
