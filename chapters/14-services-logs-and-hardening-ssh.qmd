...
{"type": "buckets", "title": "Try it: service, or just running?", "instr": "Both are processes. Sort each fact onto the column it belongs to.", "buckets": ["A service", "Just a running program"], "items": [["Started by the system at boot", 0], ["Started by a person, in a session", 1], ["Restarted if it stops", 0], ["Gone when that person logs out", 1], ["systemd knows it, from a unit file", 0], ["Appears in ps and nowhere else", 1]], "ok": "Sorted. The left column is managed and written down; the right one lives only as long as somebody keeps it alive.", "why": "A service is started at boot, restarted and known to systemd from a unit file; anything else lives in a session and appears only in ps.", "ref": "Part 1 of this chapter"}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type": "quiz", "title": "Check yourself", "q": "ss shows a database listening on 127.0.0.1:5432. Who can reach it?", "options": ["Only the machine itself", "Anyone the firewall allows in", "Every address the machine has", "Nobody, because it is stopped", "Only accounts in the adm group"], "correct": [0], "ok": "Right.", "why": "127.0.0.1 is the machine talking to itself, and nothing outside can reach it, whatever the firewall says. 0.0.0.0 is the address that faces the network.", "ref": "Part 2 of this chapter"}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type": "buckets", "title": "Try it: listening or reachable?", "instr": "A service can be listening and still unreachable. Sort each fact onto the side it describes.", "buckets": ["Listening on the machine", "Reachable from elsewhere"], "items": [["A process is waiting on a port", 0], ["Traffic actually arrives on the port", 1], ["A decision the machine made", 0], ["A decision the network made", 1], ["ss and lsof can see it", 0], ["The firewall decides it", 1]], "ok": "Sorted. Both must be true before a service is exposed.", "why": "Listening is a decision the machine made and reachable is a decision the network made; both must be true before a service is exposed.", "ref": "Part 2; Jøsang, Sect. 6.4, p. 135"}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type": "quiz", "title": "Check yourself", "q": "Which command shows only what the SSH service wrote, inside the last hour?", "options": ["journalctl -u ssh -S -1h", "journalctl -n 50", "journalctl -b", "journalctl -f", "journalctl -p err"], "correct": [0], "ok": "Right.", "why": "-u picks out one unit and -S is since. The others give the last fifty lines, this boot, a live feed, and errors and worse.", "ref": "Part 3 of this chapter"}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type": "match", "title": "Try it: three settings", "instr": "Drag each sshd_config line onto what it decides. Two lines from the shipped file are distractors.", "zones": [{"label": "May root log in over the network?", "answer": "PermitRootLogin no"}, {"label": "Is a password accepted at all?", "answer": "PasswordAuthentication no"}, {"label": "The only accounts allowed in", "answer": "AllowUsers anneli timo"}], "extra": ["X11Forwarding yes", "UsePAM yes"], "ok": "Three settings decide who may log in. Change them from a second connection, and put the file back if that one is refused.", "why": "PermitRootLogin, PasswordAuthentication and AllowUsers decide who may log in; the other two are shipped settings that do not.", "ref": "Part 4; Jøsang, Sect. 8.4"}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type": "order", "title": "Try it: survey a strange machine", "instr": "Tap the four survey commands in the order the chapter says to run them, every time.", "steps": ["ps -eo user,pid,comm - what runs, as whom", "ss -tlnp - what listens, which program", "systemctl list-units - what starts itself", "journalctl -S -1h - the last hour"], "ok": "Four answers, written down as four short lists before you change anything at all.", "why": "What runs, what listens, what starts itself, what the last hour wrote - four answers written down before you change anything.", "ref": "Part 5 of this chapter"}
</script>
```
