...
{"type":"quiz","title":"Check yourself","q":"Why does hiding the network name not hide the network?","options":["Every device that saved it asks for it by name wherever it goes","Hidden networks cannot use encryption","The name is still printed on the router","Hiding only works on the 5 GHz band"],"correct":[0],"ok":"Right.","why":"The announcement moves from the ceiling into every pocket that saved it - a readable identifier in the open, which is also why MAC filtering does not protect."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"order","title":"Try it: four generations","instr":"Put the Wi-Fi security generations in the order they arrived.","steps":["WEP","WPA","WPA2","WPA3"],"ok":"1999 to 2018 - and only the last is sound."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"quiz","title":"Check yourself","q":"An employee leaves an organisation running WPA2-Personal. What has to happen for their access to end?","options":["The shared password changes for everybody","One account is disabled, and nobody else notices","Their device forgets the key by itself","The access points must be replaced"],"correct":[0],"ok":"Right.","why":"Personal mode means one password for everybody, so revoking one person means changing it for all - which is why nobody ever changes it. Revocation is the whole argument for Enterprise mode."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"quiz","title":"Check yourself","q":"Why does a longer password not defend against an evil twin?","options":["The fake network never needed to know the password","Long passwords slow down the handshake","SAE ignores the password entirely","Evil twins can only imitate open networks"],"correct":[0],"ok":"Right.","why":"An open twin, or one that lets the handshake fail and offers a captive portal, gets the device anyway. What defends is the device checking the network's identity."}
</script>
```
