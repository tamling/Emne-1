...
```{=html}
<script type="application/json" class="qex">
{"type":"order","title":"Try it: four messages","instr":"Tap the DHCP messages in the order they are sent (tap a step, then a slot - or just tap steps in order).","steps":["Discover","Offer","Request","Acknowledge"],"ok":"Four messages and a second of time, every time a machine joins a network."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"order","title":"Try it: read the name from the right","instr":"Put the parts of www.uio.no in the order you read them.","steps":["no - the top of the tree","uio - one organisation under it","www - one machine or service they run"],"ok":"Rightmost first: that is how the inverted tree is read."}
</script>
```
...
```{=html}
<script type="application/json" class="qex">
{"type":"quiz","title":"Check yourself","q":"Why did the attack on Dyn in 2016 take Twitter offline without touching Twitter's servers?","options":["Dyn ran the name service, and a name nobody can resolve is a site nobody reaches","The Mirai botnet infected Twitter's own routers","Poisoned DNS answers sent users to wrong addresses","Twitter's servers were flooded with traffic"],"correct":[0],"ok":"Right.","why":"Name resolution is a service like any other. When it fails, everything that depends on a name fails with it, however healthy the servers behind the name are."}
</script>
```
